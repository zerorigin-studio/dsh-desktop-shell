...
// 用 HTTP 探测选出真正的 web 端口（0.1.2-alpha.1 起 web 有 URL token 认证：
// 无 token 访问 / 返回 401——401 也证明 HTTP 服务存在，同样算命中）
...
function waitForReadinessUrl(maxMs = 60000) {
  return new Promise((resolve) => {
    ...
    const cleanup = () => process.stdout.off?.('data', onData)
...
export function apply(ctx) {
  ...
  void (async () => {
    ...
    try {
      // 先等 readiness 行（web 认证 token 只随 `dsh web: <url>` 暴露）
      const authUrl = await waitForReadinessUrl()
      const marker = path.join(os.homedir(), '.dsh', 'dsh-web-port.json')
