...
function cleanupShortcuts() {
  ...
  // 但只删自己写的那份：独立客户端形态（owner=client）自己拉起 dsh 后也会写
  // 这份文件，插件卸载不该把它的状态一起清掉。
  try {
    const marker = path.join(os.homedir(), '.dsh', 'dsh-web-port.json')
    if (fs.existsSync(marker)) {
      let owner = 'plugin' // 历史文件没有 owner 字段，按插件自己写的处理
      try {
        const parsed = JSON.parse(fs.readFileSync(marker, 'utf8'))
        if (parsed && typeof parsed.owner === 'string' && parsed.owner) owner = parsed.owner
      } catch { /* 内容损坏时按历史行为处理 */ }
      if (owner === 'plugin') {
        fs.unlinkSync(marker)
        console.log('[dsh-desktop-shell] 已删除协议文件（桌面客户端将自动清理快捷方式）')
      } else {
        console.log('[dsh-desktop-shell] 协议文件由 ' + owner + ' 管理，保留不删')
      }
    }
  } catch (e) { /* ignore */ }
}
...
export function apply(ctx) {
  ...
  void (async () => {
    ...
    try {
      ...
      fs.writeFileSync(marker, JSON.stringify({
        // owner 标识写入者：插件卸载时只清理自己写的那份，
        // 独立客户端形态（owner=client）留下的状态不能被误删。
        owner: 'plugin',
  ...
  try {
    ...
    const applyAutostart = (v) => {
      ...
      try {
        // windowsHide: true —— 否则每次插件 apply（dsh 启动、插件重载）都会闪一个
      // reg.exe 控制台窗口。stdio 为 ignore 只吞掉输出，并不阻止 Windows 建控制台。
      spawn('reg.exe', cmd, { stdio: 'ignore', windowsHide: true }).on('error', () => {})
